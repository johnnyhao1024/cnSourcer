...
async function requestJson(url, init = {}) {
    try {
        ...
        if (response.status === 404) {
            return { available: false, response: null, data: null };
        }
...
export async function refreshSession() {
    ...
    const result = await requestJson(API_PATHS.session, {
        method: "GET",
        cache: "no-store"
    });

    backendAuthAvailable = result.available;
    if (!result.available) {
        return localSession;
    }

    const serverSession = result.data?.session || null;
    if (serverSession) {
        saveSession(serverSession);
        return serverSession;
    }

    if (localSession?.role === "member") {
        return localSession;
    }
...
async function loginAdminViaBackend({ username, password }) {
    const result = await requestJson(API_PATHS.login, {
        method: "POST",
    ...
    if (!result.response?.ok) {
        throw new Error(result.data?.error || "Incorrect admin username or password.");
    }
...
export async function loginUser({ username, password }) {
    ...
    if (normalized === ADMIN_USERNAME) {
        const backendSession = await loginAdminViaBackend({ username: normalized, password });
        if (backendSession) {
...
export async function logoutUser() {
    if (backendAuthAvailable || isAdminSession()) {
        await requestJson(API_PATHS.logout, { method: "POST" });
    }
    saveSession(null);
}
